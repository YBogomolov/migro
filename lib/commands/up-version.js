...
module.exports = async function (db, command) {
  ...
  const currentVersionUnappliedMigrations = await pathUtils.getUnappliedMigrations.bind(this)(versionsPath, lastMigration.version, lastMigration.name, lastMigration.version);
  ...
  for (let migration of migrationsToApply) {
    console.log(`Running migration for version ${versionToApply}: ${migration}`);
...
module.exports.command = 'up-version <db>';
