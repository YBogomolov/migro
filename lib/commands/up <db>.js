...
module.exports = async function (db, command) {
  ...
  const driver = this.drivers.get(command.parent.driver, config); // new (command.parent.drivers[command.parent.driver])(config);
  await driver.init();
  ...
  for (let version of versionsToApply) {
    ...
    let files = (await this.fs.readdirAsync(currentVersionPath))
      .filter(file => JS_FILE_REGEX.test(file))
      .filter(file => file > (lastMigration.name || ''));

    for (let migration of files) {
      console.log('Running migration', version, ':', migration);
