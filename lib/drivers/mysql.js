...
class MySQLDriver {
  ...
  async init () {
    try {
      ...
      const res = await this._mysql.queryAsync(queries.mysql.CHECK_MIGRATIONS_TABLE_EXISTENCE);
  ...
  async getLastNMigrations (count) {
    ...
    const res = await this._mysql.queryAsync(queries.mysql.GET_LAST_N_MIGRATIONS, [count]);
